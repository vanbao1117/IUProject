...
namespace IU.Web.Controllers
{
    ...
    public class LecturerController : ApiController
    {
        ...
        public async Task<IHttpActionResult> GetLecturerPreview(string classID, string subjectID, string lectureID = null)
        {
            try
            {
                using (LecturerService _LecturerService = new LecturerService())
                {
                    ...
                    var _Lecturers = await _LecturerService.GetLecturerPreviewSync(userName, classID, subjectID, lectureID);
