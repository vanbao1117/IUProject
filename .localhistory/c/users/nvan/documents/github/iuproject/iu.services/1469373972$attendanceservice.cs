...
namespace IU.Services
{
    public class AttendanceService : IDisposable
    {
        ...
        public async Task<UserAttendanceViewModel[]> GetAttendanceByStudentSync(string userId)
        {
            using (var context = new IUContext())
            {
                return await Task.Run(() => GetAttendanceByStudent(userId));
            }
        }

        private UserAttendanceViewModel[] GetAttendanceByStudent(string userId)
        {
            using (var context = new IUContext())
            {

                var semesters =
                 from semesterTBLs in context.AttendanceTBLs
