    ...
    public class AttendanceService : IDisposable
    {
        ...
        private UserAttendanceViewModel[] GetAttendanceByStudent(int pageNumber, int pageSize, string userId)
        {
            using (var context = new IUContext())
            {
                ...
                var firstPage = firstPageData.Select(f => new UserAttendanceViewModel() { Attendance = f.Attendance, AttendanceID = f.AttendanceID, Attendancer = f.Attendancer, AttendancerName =  });
        ...
        private StudentTBL GetLecturer(string userId)
        {
            using (var context = new IUContext())
            {
                var user = context.LecturerTBLs.Where(u => u.UserID == userId).FirstOrDefault();
