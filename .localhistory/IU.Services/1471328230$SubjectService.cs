    ...
    public class SubjectService : IDisposable
    {
        ...
        private ClassSchedulePageViewModel GetClassScheduleAdmin(int pageNumber, int pageSize, string classID)
        {
            using (var context = new IUContext())
            {
                ...
                var firstPage = firstPageData.ToList().Select(f => new ClassScheduleViewModel() { ClassID = f.ClassID, ClassName = GetClassName(f.ClassID), ClassScheduleID = f.ClassScheduleID, LecturerID = f.LecturerID, Lecturer = GetLecturerName(f.LecturerID), DateStudy = f.DateStudy.ToString("dddd, dd MMMM yyyy"), RoomID = f.RoomID, SlotID = f.SlotID, SubjectID = f.SubjectID, SubjectName = GetSubjectName(f.SubjectID) });
