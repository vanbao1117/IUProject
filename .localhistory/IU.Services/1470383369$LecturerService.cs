    ...
    public class LecturerService : IDisposable
    {
        ...
        private PreviewListViewModel GetLecturerPreview(string userName, string classID, string subjectID)
        {
            using (var context = new IUContext())
            {
                ...
                var Columns = from i in previewViewModels
                              group i.StudentName by i.Status;
                Dictionary<int, List<string>> Rows = new Dictionary<string, List<string>>();
                ...
                int RowIndex;
                foreach (IGrouping<string, string> c in Columns)
                {
                    Rows[0].Add(c.Key.ToString());
                    RowIndex = 1;
                    foreach (string status in c)
                    {
                        Rows[RowIndex].Add(status);
                        RowIndex++;
                    }
                ...
                PreviewListViewModel model = new PreviewListViewModel() { ClassID = classID, SubjectID = subjectID, Preview = previewViewModels, Rows = Rows };
                return model;
            }
        }
