                    ...
                    foreach (DateTime dateStudy in studyDates)
                    {
                        ...
                        foreach (StudentListTBL studentList in studentLists)
                        {
                            if((model.SlotID2 == null){
                                 var classSchedule = new ClassScheduleTBL() { ClassScheduleID = Helper.GenerateRandomId(), ClassID = model.ClassID, RoomID = model.RoomID, SlotID = model.SlotID1, SubjectID = model.SubjectID.Replace("/r/n", string.Empty), LecturerID = model.LecturerID, StudentListID = studentList.StudentListID, IsAttendance = false, ModeID = model.ModeID, Blog = model.BlogID, DateStudy = dateStudy };
                                ClassScheduleTBLRepository.Save(classSchedule);
                            }else{
                                 var classSchedule = new ClassScheduleTBL() { ClassScheduleID = Helper.GenerateRandomId(), ClassID = model.ClassID, RoomID = model.RoomID, SlotID = model.SlotID1 + (model.SlotID2.Equals("") ? "" : "-" + model.SlotID2), SubjectID = model.SubjectID.Replace("/r/n", string.Empty), LecturerID = model.LecturerID, StudentListID = studentList.StudentListID, IsAttendance = false, ModeID = model.ModeID, Blog = model.BlogID, DateStudy = dateStudy };
                                ClassScheduleTBLRepository.Save(classSchedule);
                            }
