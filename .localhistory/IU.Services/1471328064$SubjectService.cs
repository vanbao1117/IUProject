    ...
    public class SubjectService : IDisposable
    {
        ...
        private ClassSchedulePageViewModel GetClassScheduleAdmin(int pageNumber, int pageSize, string classID)
        {
            using (var context = new IUContext())
            {
                ...
                var firstPage = firstPageData.ToList().Select(f => new ClassScheduleViewModel() { ClassID = f.Key.ClassID, ClassName = GetClassName(f.Key.ClassID), ClassScheduleID = f.Key.ClassScheduleID, LecturerID = f.Key.LecturerID, Lecturer = GetLecturerName(f.Key.LecturerID), DateStudy = f.Key.DateStudy.ToString("dddd, dd MMMM yyyy"), RoomID = f.Key.RoomID, SlotID = f.Key.SlotID, SubjectID = f.Key.SubjectID, SubjectName = GetSubjectName(f.Key.SubjectID) });
