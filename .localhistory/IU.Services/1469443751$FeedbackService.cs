...
namespace IU.Services
{
    public class FeedbackService : IDisposable
    {
        ...
        public async Task<FeedbackViewModel> SubmitFeedbackSync(FeedbackViewModel model, string userId)
        {
            ...
            {
                var student = context.StudentTBLs.Where(s => s.UserID == userId);
                string _id = Helper.GenerateRandomId();
                new FeedBackTBL() { Id = _id, Attitude = model.Attitude, Comments = model.Comments, Lecturer = model.LecturerID, OnTime = model.OnTime, Quality = model.Quality, Satisfaction = model.Satisfaction, Student = studentId };
