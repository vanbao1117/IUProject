...
namespace IU.Services
{
    public class FeedbackService : IDisposable
    {
        ...
        private FeedbackViewModel SubmitFeedback(FeedbackViewModel model, string userId)
        {
            using (var context = new IUContext())
            {
                string _id = Helper.GenerateRandomId();
                await FeedbackRepository.SaveAsync(new FeedBackTBL() { Id = _id, TransCode = model.TransCode, TransName = model.TransName });
                model.Id = _id;
                return model;
                return feedback;
            }
