            ...
            try
            {
                using (var context = new IUContext())
                {
                    ...
                    foreach (DateTime dateStudy in studyDates)
                    {
                        ...
                        {
                            var classSchedule = new ClassScheduleTBL() { ClassScheduleID = Helper.GenerateRandomId(), ClassID = model.ClassID, RoomID = model.RoomID, SlotID = model.SlotID, SubjectID = model.SubjectID, LecturerID = model.LecturerID, StudentListID = studentList.StudentListID, IsAttendance = false, ModeID = model.ModeID, Blog = 1, DateStudy = dateStudy };
            ...
            try
            {
                ...
                using (var context = new IUContext())
                {
                    var sh = from t in (
                       ...
                       group t by new { t.h.ClassID, t.h.LecturerID, t.h.SubjectID, t.h.RoomID, t.h.SlotID , t.h.ModeID, t.h.Blog} into g
                       select new ClassScheduleViewModel(){
                            ...
                            RoomID = g.Key.RoomID,
                            SlotID = g.Key.SlotID ,
                            ModeID = g.Key.ModeID.Value,
                            BlogID = g.Key.Blog.Value,
                            SemesterID = semesterID,
                            DateStudy = Helper.GetStudyDays(sem.StartDate.Value, sem.EndDate.Value, g.Key.ModeID.Value, g.Key.Blog.Value)[0],
                            isCreate = false
                      };
