                    ...
                    foreach (DateTime dateStudy in studyDates)
                    {
                        ...
                        foreach (StudentListTBL studentList in studentLists)
                        {
                            ...
                            if(model.SlotID2 == null){
                                var classSchedule = new ClassScheduleTBL() { ClassScheduleID = Helper.GenerateRandomId(), ClassID = model.ClassID.Replace("/r/n", string.Empty), RoomID = model.RoomID, SlotID = GetSlot(model.SlotID1), SubjectID = model.SubjectID.Replace("/r/n", string.Empty), LecturerID = model.LecturerID, StudentListID = studentList.StudentListID, IsAttendance = false, ModeID = model.ModeID, Blog = model.BlogID, DateStudy = dateStudy };
