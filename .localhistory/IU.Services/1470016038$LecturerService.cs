    ...
    public class LecturerService : IDisposable
    {
        ...
        private List<UserAttendanceViewModel> GetAttendanceTwoDaysBefore(string userName)
        {
            using (var context = new IUContext())
            {
                ...
                var results = context.AttendanceTBLs.Where(x => x.Attendancer == user.Id && x.SemesterID == semesterID).GroupBy(x => new { x.SubjectID, x.ClassID, x.RoomID, x.SlotID, x.Note, x.AttendanceID }).Select(x.SubjectID, x.ClassID, x.RoomID, x.SlotID, x.Note, x.AttendanceID).Distinct();

                var attendances = results.ToList().Select(a => new UserAttendanceViewModel() { SubjectID = GetSubjectName(a.)  });
