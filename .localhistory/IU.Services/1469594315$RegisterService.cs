...
namespace IU.Services
{
    public class RegisterService : IDisposable
    {
        ...
        public async Task<AcceptRegister> AcceptRegisterSync(OpenSubjectViewModel model, string userName)
        {
            using (var context = new IUContext())
            {
                try
                {
                    ...
                    await AcceptRegisterRepository.SaveAsync(new FeedBackTBL() { Id = _id, Attitude = model.Attitude, Comments = model.Comments, Lecturer = model.LecturerID, OnTime = model.OnTime, Quality = model.Quality, Satisfaction = model.Satisfaction, Student = student.StudentID, FeedbackDate = DateTime.Now });
                    return model;
                }
                catch (Exception ex) { }
                return null;
