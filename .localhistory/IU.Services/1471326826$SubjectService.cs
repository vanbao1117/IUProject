    ...
    public class SubjectService : IDisposable
    {
        ...
        private ClassSchedulePageViewModel GetClassScheduleAdmin(int pageNumber, int pageSize, string classID)
        {
            using (var context = new IUContext())
            {
                ...
                var classSheduleTbl = context.ClassScheduleTBLs.Where(c => c.ClassID == classID.Replace("\r\n", string.Empty)).GroupBy(f => new { f.ClassID, f.ClassScheduleID, f.LecturerID, RoomID = f.RoomID, SlotID = f.SlotID, f.SubjectID, f.DateStudy }).Distinct();

                var firstPageData = Helper.PagedResult(classSheduleTbl, pageNumber, pageSize, classScheduleTBLs => classScheduleTBLs.FirstOrDefault().DateStudy, false, out NumberOfItems);
