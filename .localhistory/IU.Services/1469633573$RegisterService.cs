            ...
            using (var context = new IUContext())
            {
                ...
                foreach (OpenSubjectViewModel model in openSubjects.ToArray())
                {
                    ...
                    var openClass = context.OpenClassTBLs.Where(o => o.OpenClassID == model.OpenClassID).ToArray()
                        .Select(o => new OpenClassViewModel() { Active = checkExitOpenClass(o.OpenClassID) && index == 0 ? true : false, ClassID = o.ClassID, ClassName = GetClass(o.ClassID).ClassName, CreatedDate = o.CreatedDate, Creater = o.Creater, OpenClassID = o.OpenClassID, RoomID = o.RoomID, RoomName = o.RoomID, Select = checkExitOpenClass(o.OpenClassID), SlotID = o.SlotID, SlotName = GetSlotbyID(o.SlotID), StartDate = GetClass(o.ClassID).StartDate, Accepted = GetAcceptRegister(model.OpenClassID) == null ? false : GetAcceptRegister(model.OpenClassID).Accepted.Value, Status = GetAcceptRegister(model.OpenClassID) == null ? "" : GetAcceptRegister(model.OpenClassID).Accepted.Value ? "Accepted" : "Pending" }).ToList();
