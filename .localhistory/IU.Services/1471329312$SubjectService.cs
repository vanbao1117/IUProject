    ...
    public class SubjectService : IDisposable
    {
        ...
        private ClassSchedulePageViewModel GetClassScheduleAdmin(int pageNumber, int pageSize, string classID)
        {
            using (var context = new IUContext())
            {
                ...
                var classSheduleTbl = context.ClassScheduleTBLs.GroupBy(f => new { f.ClassID, f.ClassScheduleID, f.LecturerID, f.RoomID, f.SlotID, f.SubjectID, f.DateStudy }).Select(g => g.First()).ToList()
                .Select(f => new {
