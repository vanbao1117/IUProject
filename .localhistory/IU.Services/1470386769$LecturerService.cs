    ...
    public class LecturerService : IDisposable
    {
        ...
        private PreviewListViewModel GetLecturerPreview(string userName, string classID, string subjectID)
        {
            using (var context = new IUContext())
            {
                ...
                var previewViewModels = attandances.Select(a => new PreviewViewModel() { Day = a.DateAttendance.Day + "/" + a.DateAttendance.Month, Slot = GetSlot(a.SlotID).SlotName, Status = (a.Attendance == true? "P" : "A"), StudentName = GetStudent(a.StudentListID).StudentName }).ToList();
                ...
                PreviewListViewModel model = new PreviewListViewModel() { ClassID = classID, SubjectID = subjectID, Preview = previewViewModels, Data = new List<LecturePreviewViewModel>() };

                foreach (var studnt in rows)
                {
                    
                    var _cols = previewViewModels.Where(p => p.StudentName == studnt.StudentName).ToList();
                    LecturePreviewViewModel row = new LecturePreviewViewModel() { Row = studnt.StudentName, Cols = _cols, CurrentSemmeter = GetCurrentSemester().SemesterName };
