    ...
    public class LecturerService : IDisposable
    {
        ...
        private PreviewListViewModel GetLectureSubjectAndClass(string userName, string classID, string subjectID)
        {
            using (var context = new IUContext())
            {
                ...
                var previewViewModels = attandances.Select(a => new PreviewViewModel() { Day = a.DateAttendance.Day + "/" + a.DateAttendance.Month, Slot =  }).ToList();
        ...
        private SlotTBL GetSlotbyID(string SlotID)
        {
            using (var context = new IUContext())
            {
