...
namespace IU.Services
{
    public class RegisterService : IDisposable
    {
        ...
        private bool CreateClassSchedule2(ClassScheduleViewModel model, string userName)
        {
            try
            {
                using (var context = new IUContext())
                {
                    ...
                    {
                        var studentLists = StudentListRepository.FindAllBy(s => s.ClassID == model.ClassID && s.SemesterID == model.SemesterID).ToList();
        ...
        private void createClassSchedule(AcceptRegister accept)
        {
            using (var context = new IUContext())
            {
                ...
                var sm = GetCurrentSemester();
                DateTime[] studyDates = Helper.GetStudyDays(sm.StartDate.Value, sm.EndDate.Value, openSubject.ModeID.Value.ToString(), 1);
                foreach (DateTime dateStudy in studyDates)
                {
                    if (dateStudy.Year == openSubject.StartDate.Value.Year && dateStudy.Month == openSubject.StartDate.Value.Month && dateStudy.Day == openSubject.StartDate.Value.Day)
                    {
                        break;
                    }
