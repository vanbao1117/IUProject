    ...
    public class RegisterService : IDisposable
    {
        ...
        private void createClassSchedule(AcceptRegister accept)
        {
            using (var context = new IUContext())
            {
                ...
                DateTime[] studyDatesBlog1 = Helper.GetStudyDays(sm.StartDate.Value, sm.EndDate.Value, openSubject.ModeID.Value.ToString(), 1);
                DateTime[] studyDatesBlog2 = Helper.GetStudyDays(sm.StartDate.Value, sm.EndDate.Value, openSubject.ModeID.Value.ToString(), 2);

                int blog = 1;
                if (studyDatesBlog2.Contains(openSubject.StartDate.Value))
                {
                    blog = 2;
                }
                ...
                if (blog == 1)
                {
                    totalW = Enumerable.Range(0, (int)(studyDatesBlog1[studyDatesBlog1.Length - 1] - openSubject.StartDate.Value).TotalDays + 1)
                                    .Select(d => sm.StartDate.Value.AddDays(d))
                                    .Where(d => d.DayOfWeek == DayOfWeek.Sunday).ToArray();
                }
                else
                {
                    totalW = Enumerable.Range(0, (int)(studyDatesBlog2[studyDatesBlog2.Length - 1] - openSubject.StartDate.Value).TotalDays + 1)
                                    .Select(d => sm.StartDate.Value.AddDays(d))
                                    .Where(d => d.DayOfWeek == DayOfWeek.Sunday).ToArray();
                }
                ...
                DateTime[] m24 = Helper.GetStudyDays(Helper.FirstDayOfWeek(w1), w3, "2-4", blog);
                lsStudyDate.AddRange(m24);

                DateTime[] m35 = Helper.GetStudyDays(Helper.FirstDayOfWeek(w1), w3, "3-5", blog);
                lsStudyDate.AddRange(m35);

                //2-4-6, 3-5-6
                DateTime w4 = totalW[3];
                DateTime w5 = totalW[4];
                DateTime w6 = totalW[5];

                DateTime[] m246 = Helper.GetStudyDays(Helper.FirstDayOfWeek(w1), w3, "2-4-6", blog);
                lsStudyDate.AddRange(m246);

                DateTime[] m356 = Helper.GetStudyDays(Helper.FirstDayOfWeek(w1), w3, "3-5-6", blog);
                lsStudyDate.AddRange(m356);


                foreach (DateTime dateStudy in lsStudyDate)
                {
                    if (dateStudy <= openSubject.StartDate.Value)
                    {
                        continue;
                    }
