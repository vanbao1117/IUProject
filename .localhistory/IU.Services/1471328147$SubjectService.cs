    ...
    public class SubjectService : IDisposable
    {
        ...
        private ClassSchedulePageViewModel GetClassScheduleAdmin(int pageNumber, int pageSize, string classID)
        {
            using (var context = new IUContext())
            {
                ...
                var classSheduleTbl= context.ClassScheduleTBLs.GroupBy(f => new { f.ClassID, f.ClassScheduleID, f.LecturerID, f.RoomID, f.SlotID, f.SubjectID, f.DateStudy}).Distinct()
                .Select(f => new ClassScheduleViewModel()
                {
                    ClassID = f.Key.ClassID,
                    ClassScheduleID = f.Key.ClassScheduleID,
                    LecturerID = f.Key.LecturerID,
                    RoomID = f.Key.RoomID,
                    SlotID = f.Key.SlotID,
                    SubjectID = f.Key.SubjectID,
                    DateStudy = f.Key.DateStudy.ToString("dddd, dd MMMM yyyy")
                      });

                var firstPageData = Helper.PagedResult(classSheduleTbl, pageNumber, pageSize, classSheduleTbl => classSheduleTbl.DateStudy, false, out NumberOfItems);

                var firstPage = firstPageData.ToList().Select(f => new ClassScheduleViewModel() { ClassID = f.Key.ClassID, ClassName = GetClassName(f.Key.ClassID), ClassScheduleID = f.Key.ClassScheduleID, LecturerID = f.Key.LecturerID, Lecturer = GetLecturerName(f.Key.LecturerID), DateStudy = f.Key.DateStudy, RoomID = f.Key.RoomID, SlotID = f.Key.SlotID, SubjectID = f.Key.SubjectID, SubjectName = GetSubjectName(f.Key.SubjectID) });
