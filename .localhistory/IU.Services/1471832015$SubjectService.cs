...
namespace IU.Services
{
    public class SubjectService : IDisposable
    {
        ...
        public async Task<bool> ValidateScheduleSync(string semesterID, string dateStudy, int mode, int blog, string room, string slot)
        {
            using (var context = new IUContext())
            {
                return await Task.Run(() => ValidateSchedule( semesterID,  dateStudy,  mode,  blog,  room,  slot));
            }
        }
        ...
        private bool CreateClassSchedule(ClassScheduleViewModel model, string userName)
        {
            try
            {
                ...
                using (var context = new IUContext())
                {
                    ...
                    ValidateSchedule(model.SemesterID, model.DateStudy, int.Parse(mode.Mode), model.BlogID, model.RoomID, model.SlotID);
