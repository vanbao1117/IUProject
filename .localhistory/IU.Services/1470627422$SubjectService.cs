...
namespace IU.Services
{
    public class SubjectService : IDisposable
    {
        ...
        private bool CreateBis(BisViewModel model)
        {
            ...
            OpenClassRepository.Save(new OpenClassTBL() { OpenClassID = openClassID, ClassID = classID, CreatedDate = DateTime.Now, Creater = "", Deadline = model.Deadline, Limit = model.Limit, RoomID = model.RoomID, SemesterID = model.SemesterID, SlotID = model.SlotIDs.Join("-", model.SlotIDs) });
            OpenSubjectTBLRepository.Save(new OpenSubjectTBL() { OpenSubjectID = Helper.GenerateRandomId(), OpenClassID = openClassID, LecturerID = model.LecturerID, ModeID = model.ModeID, SubjectID = model.SubjectID, StartDate = model.StartDate, Cost = 0, CreatedDate = DateTime.Now, Creater = "admin", Credit = 0 });
            return true;
        }
