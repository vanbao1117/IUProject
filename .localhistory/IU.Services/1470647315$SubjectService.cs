...
namespace IU.Services
{
    public class SubjectService : IDisposable
    {
        ...
        public async Task<bool> GetClassScheduleSync(string classID, string semesterID)
        {
            using (var context = new IUContext())
            {
        ...
        private bool GetClassSchedule(string classID, string semesterID)
        {
            try
            {
                using (var context = new IUContext())
                {
                    var results = context.ClassScheduleTBLs.Join(
                        context.StudentListTBLs,
                            classScheduleTBLs => classScheduleTBLs.StudentListID,
                            studentListTBLs => studentListTBLs.StudentListID,
                            (classScheduleTBLs, studentListTBLs) => new { classScheduleTBLs, studentListTBLs }
                    )
                    .Where(fullEntry => fullEntry.studentListTBLs.ClassID == classID && fullEntry.studentListTBLs.SemesterID == semesterID)
                    .GroupBy(x => new { x.ClassID, x.LecturerID, x.SubjectID, x.RoomID, x.SlotID }, (key, group) => new
                       {
                           ClassID = key.ClassID,
                           LecturerID = key.LecturerID,
