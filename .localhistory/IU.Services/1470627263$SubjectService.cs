...
namespace IU.Services
{
    public class SubjectService : IDisposable
    {
        ...
        private bool CreateBis(BisViewModel model)
        {
            ...
            OpenClassRepository.Save(new OpenClassTBL() { OpenClassID = openClassID, ClassID = classID, CreatedDate = DateTime.Now, Creater = "", Deadline = model.Deadline, Limit = model.Limit, RoomID = model.RoomID, SemesterID = model.SemesterID, SlotID = model.SlotIDs.Join("-") });
