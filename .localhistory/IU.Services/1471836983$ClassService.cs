    ...
    public class ClassService : IDisposable
    {
        ...
        public async Task<bool> ChangeClass(string oldClassID, string studentID, string newClassID)
        {
            try
            {
                ...
                StudentListTBLRepository.Save(new StudentListTBL() { StudentListID = Helper.GenerateRandomId(), ClassID = newClassID, SemesterID = sem.SemesterID, StudentID = student.StudentID });
                return true;
            }
            catch (Exception ex)
            {
