...
namespace IU.Services
{
    public class SubjectService : IDisposable
    {
        ...
        public async Task<ClassSchedulePageViewModel> GetClassScheduleAdminSync(string classID)
        {
            using (var context = new IUContext())
            {
                return await Task.Run(() => GetClassScheduleAdmin(classID));
            }
        }

        private ClassSchedulePageViewModel GetClassScheduleAdmin(string classID)
        {
            using (var context = new IUContext())
            {
                int NumberOfItems = 0;
                ...
                var firstPageData = Helper.PagedResult(classSheduleTbl, 1, 200, classScheduleTBLs => classScheduleTBLs.DateStudy, false, out NumberOfItems);

                var firstPage = firstPageData.ToList().Select(f => new ClassScheduleViewModel() { ClassID = f.ClassID, ClassName = GetClassName(f.ClassID), ClassScheduleID = f.ClassScheduleID, LecturerID = f.LecturerID, Lecturer = GetLecturerName(f.LecturerID), DateStudy = f.DateStudy.ToString("dddd, dd MMMM yyyy"), RoomID = f.RoomID, SlotID = f.SlotID, StudentID = GetStudent(f.StudentListID).StudentID, StudentListID = f.StudentListID, StudentName = GetStudent(f.StudentListID).StudentName, SubjectID = f.SubjectID, SubjectName = GetSubjectName(f.SubjectID) });

                int totalPage = (int)Math.Ceiling((double)NumberOfItems / (double)200);
                ...
                using (var context = new IUContext())
                {
                    ...
                    var newSH = from h in context.SemesterClassSubjectTBLs
                                ...
                                select new ClassScheduleViewModel()
                                {
                                    ...
                                    DateStudy = DateTime.Now.ToString("dddd, dd MMMM yyyy"),
                                    isCreate = true
                                };
                    if (newSH != null)
                    {
