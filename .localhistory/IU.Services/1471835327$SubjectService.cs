            ...
            try
            {
                using (var context = new IUContext())
                {
                    ...
                    foreach (string slot in model.SlotIDs)
                    {
                        ...
                        var isValid = ValidateSchedule(model.SemesterID, model.StartDate.ToString(), 8, -1, model.RoomID, slot);
                        if (!isValid) return false;

                        //check exist slot for lecture same day
                        var isValidLecture = ValidateLectureSchedule(model.SemesterID, model.StartDate.ToString(), 8, -1, model.RoomID, slot, model.LecturerID);
