...
namespace IU.Services
{
    public class SubjectService : IDisposable
    {
        ...
        public async Task<ClassSchedulePageViewModel> GetClassScheduleSync(string classID)
        {
            using (var context = new IUContext())
            {
                return await Task.Run(() => ClassSchedulePageViewModel(classID));
            }
        }

        private ClassSchedulePageViewModel GetClassSchedule(string classID)
        {
            using (var context = new IUContext())
            {
                int NumberOfItems = 0;
