        ...
        private PreviewListViewModel GetLecturerPreview(string userName, string classID, string subjectID)
        {
            using (var context = new IUContext())
            {
                ...
                foreach (var studnt in rows)
                {
                    ...
                    var _cols = previewViewModels.Where(p => p.StudentName == studnt.StudentName.ToString()).ToList();
                    LecturePreviewViewModel row = new LecturePreviewViewModel() { Row = studnt.ToString(), Cols = _cols };
                    model.Data.Add(row);
                }
