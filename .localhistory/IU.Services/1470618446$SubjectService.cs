...
namespace IU.Services
{
    public class SubjectService : IDisposable
    {
        ...
        public async Task<List<RoomViewModel>> GetAllSlotsSync()
        {
            using (var context = new IUContext())
            {
        ...
        private List<SlotViewModel> GetAllSlots()
        {
            using (var context = new IUContext())
            {
                var slots = context.SlotTBLs.Select(s => new SlotViewModel() { SlotID = s.SlotID, SlotName = s.SlotName, SlotTime = s.SlotTime, NumOfSlot = s.NumOfSlot, TotalSlot = s.TotalSlot });
                return slots.ToList();
            }
        }
