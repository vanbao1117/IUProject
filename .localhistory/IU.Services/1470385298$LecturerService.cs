        ...
        private PreviewListViewModel GetLecturerPreview(string userName, string classID, string subjectID)
        {
            using (var context = new IUContext())
            {
                ...
                foreach (var studnt in rows)
                {
                    ...
                    var cols = previewViewModels.Where(p => p.StudentName == studnt.ToString()).ToList();
                    LecturePreviewViewModel row = new LecturePreviewViewModel() { Row = studnt.ToString(), Cols = cols };
                    model.Data.Add(row);
                }
