            ...
            using (var context = new IUContext())
            {
                ...
                foreach (DateTime date in dates)
                {
                    var results = context.AttendanceTBLs.Where(x => x.Attendancer == user.Id && x.SemesterID == semesterID && x.DateAttendance.Year == date.Year
                       ...
                       && x.DateAttendance.Day == date.Day).GroupBy(x => new { x.SubjectID, x.ClassID, x.RoomID, x.SlotID, x.Note, x.DateAttendance }, (key, group) => new
                       {
                           ClassID = key.ClassID,
                           RoomID = key.RoomID,
                           ...
                           SlotID = key.SlotID,
                           Note = key.Note,
                           DateAttendance = key.DateAttendance,
                           Result = group.ToList()
                       }).ToList().Distinct();
