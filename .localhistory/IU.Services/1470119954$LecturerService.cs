...
namespace IU.Services
{
    public class LecturerService : IDisposable
    {
        ...
        public async Task<List<UserAttendanceViewModel>> TakeAttendancesSync(List<UserAttendanceViewModel> models, string userName)
        {
        ...
        private List<UserAttendanceViewModel> TakeAttendances(List<UserAttendanceViewModel> models, string userName)
        {
            foreach (UserAttendanceViewModel model in models)
            {
                ...
                {
                    AttendanceTBLRepository.SaveAsync(new AttendanceTBL(){AttendanceID = Helper.GenerateRandomId(), Attendance = model.Present, Attendancer = })
                }
        ...
        private List<UserAttendanceViewModel> GetClassSchedule(string lecturerID, string[] studentListIDs, DateTime[] dates, IUContext context)
        {
            ...
            foreach (DateTime date in dates)
            {
                var results = context.ClassScheduleTBLs.Where(x => x.LecturerID == lecturerID && studentListIDs.Contains(x.StudentListID) && x.DateStudy.Year == date.Year
                   ...
                   && x.DateStudy.Day == date.Day).GroupBy(x => new { x.SubjectID, x.ClassID, x.RoomID, x.SlotID, x.DateStudy }, (key, group) => new
                   {
                       ClassID = key.ClassID,
                       RoomID = key.RoomID,
                       SubjectID = key.SubjectID,
