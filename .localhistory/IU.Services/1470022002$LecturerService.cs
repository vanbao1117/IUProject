...
namespace IU.Services
{
    public class LecturerService : IDisposable
    {
        ...
        private List<ClassScheduleTBL> GetClassSchedle(string lecturerID, string studentListID, DateTime[] dates, IUContext context)
        {
            var semesterID = GetCurrentSemester().SemesterID;
            foreach (DateTime date in dates)
            {
                var results = context.ClassScheduleTBLs.Where(x => x.LecturerID == lecturerID && x.StudentListID == studentListID && x.DateStudy.Year == date.Year
                   && x.DateStudy.Month == date.Month
                   && x.DateStudy.Day == date.Day).GroupBy(x => new { x.SubjectID, x.ClassID, x.RoomID, x.SlotID, x.Note }, (key, group) => new
                   {
                       ClassID = key.ClassID,
                       RoomID = key.RoomID,
                       SubjectID = key.SubjectID,
