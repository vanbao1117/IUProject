    ...
    public class LecturerService : IDisposable
    {
        ...
        private List<UserAttendanceViewModel> GetAttendanceTwoDaysBefore(string userName)
        {
            using (var context = new IUContext())
            {
                ...
                var results = context.AttendanceTBLs.Where(x => x.Attendancer == user.Id && x.SemesterID == semesterID && DateTime.Compare(x.DateAttendance, date) <= 0).GroupBy(x => new { x.SubjectID, x.ClassID, x.RoomID, x.SlotID, x.Note }, (key, group) => new
                {
                    ClassID = key.ClassID,
                    RoomID = key.RoomID,
