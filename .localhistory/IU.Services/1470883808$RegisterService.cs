    ...
    public class RegisterService : IDisposable
    {
        ...
        private void createClassSchedule(AcceptRegister accept)
        {
            using (var context = new IUContext())
            {
                ...
                if (blog == 1)
                {
                    DateTime[] totalW = Enumerable.Range(0, (int)(studyDatesBlog1[studyDatesBlog1.Length - 1] - openSubject.StartDate.Value).TotalDays + 1)
                                    .Select(d => sm.StartDate.Value.AddDays(d))
                                    .Where(d => d.DayOfWeek == DayOfWeek.Sunday).ToArray();
                }
                else
                {
                    DateTime[] totalW = Enumerable.Range(0, (int)(studyDatesBlog2[studyDatesBlog2.Length - 1] - openSubject.StartDate.Value).TotalDays + 1)
                                    .Select(d => sm.StartDate.Value.AddDays(d))
                                    .Where(d => d.DayOfWeek == DayOfWeek.Sunday).ToArray();
                }
