    ...
    public class RegisterService : IDisposable
    {
        ...
        private void createClassSchedule(AcceptRegister accept)
        {
            using (var context = new IUContext())
            {
                ...
                DateTime[] studyDates = studyDatesBlog1;
                if (studyDatesBlog2.Contains(openSubject.StartDate.Value))
                {
                    studyDates = studyDatesBlog2;
                }

                DateTime[] totalDates = Enumerable.Range(0, (int)(sm.EndDate.Value - openSubject.StartDate.Value).TotalDays + 1)
