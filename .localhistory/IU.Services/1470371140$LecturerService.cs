    ...
    public class LecturerService : IDisposable
    {
        ...
        private LectureClassSubjectViewModel GetLectureClassSubject(string userName)
        {
            using (var context = new IUContext())
            {
                ...
                var subjects = lectures.SubjectTBLs.Select(s => new UserSubjectViewModel() { SubjectID = s.SubjectID, SubjectName = s.SubjectName, AbbreSubjectName = s.AbbreSubjectName, UserId = user.Id });

                var classs = lectures.LecturerScheduleTBLs.Where(c => c.SemesterID == GetCurrentSemester().SemesterID).ToList();
                var lecClass = classs.Select(c => new ClassViewModel() { ClassID = c.ClassID, ClassName = GetClass(c.ClassID).ClassName });


                LectureClassSubjectViewModel model = new LectureClassSubjectViewModel() { LectureClass = lecClass, LectureSubject = subjects };
                return new LectureClassSubjectViewModel();
            }
        }
        ...
        private PreviewListViewModel GetLecturerPreview(string userName, string classID, string subjectID)
        {
            using (var context = new IUContext())
            {
                ...
                var previewViewModels = attandances.Select(a => new PreviewViewModel() { Day = a.DateAttendance.Day + "/" + a.DateAttendance.Month, Slot = GetSlot(a.SlotID).SlotName, Status = (a.Attendance == true? "P" : "A") }).ToList();
