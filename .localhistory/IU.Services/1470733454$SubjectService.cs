    ...
    public class SubjectService : IDisposable
    {
        ...
        private bool CreateLecturer(LecturerViewModel model, string userName)
        {
            try
            {
                ...
                var lecture = new LecturerTBL() { LecturerID = Helper.GenerateRandomId(), UserID = signupUserModel.Id, LecturerBirth = model.LecturerBirth, LecturerEmail = model.LecturerEmail, LecturerGender = model.LecturerGender, LecturerName = model.LecturerName, LecturerPhone = model.LecturerPhone };

                LecturerTBLRepository.Save(lecture);
                using (var context = new IUContext())
                {
                    context.LecturerTBLs.Include("SubjectTBLs").Where(l => l.LecturerID == lecture.LecturerID).FirstOrDefault();
                }
        ...
        private bool CreateStudent(StudentViewModel model, string userName)
        {
            try
            {
                ...
                AccountRepository.Save(signupUserModel);
                var student = new StudentTBL() { StudentID = Helper.GenerateRandomId(), ParentPhone = model.ParentPhone, StudentBirth = model.StudentBirth, StudentCode = model.StudentID, StudentEmail = model.StudentEmail, StudentGender = model.StudentGender, StudentName = model.StudentName, StudentPhone = model.StudentPhone, UserID = signupUserModel.Id };
