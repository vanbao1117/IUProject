...
namespace IU.Services
{
    public class ClassService : IDisposable
    {
        ...
        public async Task<ClassViewModel[]> GetAllClassList()
        {
            try
            {
                ClassTBLRepository.FindAll().Select(c => new ClassViewModel() { ClassID = c.ClassID, ClassName = c.ClassName, IsMain = c.IsMainClass }).ToArray();
            }
            catch (Exception ex)
            {
            }
