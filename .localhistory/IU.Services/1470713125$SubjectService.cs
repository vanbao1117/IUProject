...
namespace IU.Services
{
    public class SubjectService : IDisposable
    {
        ...
        private bool UpdateClassSchedule(ClassScheduleViewModel model, string userName)
        {
            try
            {
                var classSchedule = ClassScheduleTBLRepository.FindOneBy(c => c.ClassScheduleID == model.ClassScheduleID);
                ...
                ClassScheduleTBLRepository.UpdateAsync(classSchedule);
                return true;
            }
            catch (Exception ex)
            {
        ...
        private bool CreateClassSchedule(ClassScheduleViewModel model, string userName)
        {
            try
            {
                ...
                var studentLists = StudentListTBLRepository.FindAllBy(s=>s.ClassID == model.ClassID && s.SemesterID == model.s)
                var classSchedule = new ClassScheduleTBL() { ClassScheduleID = Helper.GenerateRandomId(), ClassID = model.ClassID, RoomID = model.RoomID, SlotID = model.SlotID, SubjectID = model.SubjectID, LecturerID = model.LecturerID, StudentListID = "", IsAttendance = false, ModeID = 5, Blog = 1, DateStudy = model.DateStudy };
            ...
            try
            {
                using (var context = new IUContext())
                {
                    var sh = from t in (
                       ...
                       group t by new { t.h.ClassID, t.h.LecturerID, t.h.SubjectID, t.h.RoomID, t.h.SlotID , t.h.DateStudy, t.h.ClassScheduleID} into g
                       select new ClassScheduleViewModel(){
                            ...
                            ClassScheduleID = g.Key.ClassScheduleID
                      };

                    var listing = sh.ToList();
