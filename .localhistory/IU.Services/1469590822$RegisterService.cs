            ...
            using (var context = new IUContext())
            {
                ...
                foreach (OpenSubjectViewModel model in openSubjects.ToArray())
                {
                    var openClass = context.OpenClassTBLs.Where(o => o.OpenClassID == model.OpenClassID).ToArray()
                        .Select(o => new OpenClassViewModel() { Active = false, ClassID = o.ClassID, ClassName = GetClass(o.ClassID), CreatedDate = o.CreatedDate, Creater = o.Creater, OpenClassID = o.OpenClassID, RoomID = o.RoomID, RoomName = o.RoomID, Select = false, SlotID = o.SlotID, SlotName = o.SlotID }).ToList();
