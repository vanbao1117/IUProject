...
namespace IU.Services
{
    public class RegisterService : IDisposable
    {
            ...
            using (var context = new IUContext())
            {
                ...
                foreach (OpenSubjectViewModel model in openSubjects.ToArray())
                {
                    var openClass = context.OpenClassTBLs.Where(o => o.OpenClassID == model.OpenClassID).ToArray()
                        .Select(o => new OpenClassViewModel() { Active = checkExitOpenClass(o.OpenClassID), ClassID = o.ClassID, ClassName = GetClass(o.ClassID).ClassName, CreatedDate = o.CreatedDate, Creater = o.Creater, OpenClassID = o.OpenClassID, RoomID = o.RoomID, RoomName = o.RoomID, Select = checkExitOpenClass(o.OpenClassID), SlotID = o.SlotID, SlotName = GetSlotbyID(o.SlotID), StartDate = GetClass(o.ClassID).StartDate }).ToList();
        ...
        private AcceptRegister GetAcceptRegister(string openClassID)
        {
            using (var context = new IUContext())
            {
                var acceptRegister = context.AcceptRegisters.Where(o => o.OpenClassID == openClassID);
