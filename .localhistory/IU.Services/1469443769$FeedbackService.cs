    ...
    public class FeedbackService : IDisposable
    {
        ...
        public async Task<FeedbackViewModel> SubmitFeedbackSync(FeedbackViewModel model, string userName)
        {
            using (var context = new IUContext())
            {
                ...
                new FeedBackTBL() { Id = _id, Attitude = model.Attitude, Comments = model.Comments, Lecturer = model.LecturerID, OnTime = model.OnTime, Quality = model.Quality, Satisfaction = model.Satisfaction, Student = student.StudentID };
