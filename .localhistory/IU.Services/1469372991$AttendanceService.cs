        ...
        private UserSemesterViewModel[] GetSemesterByStudent(string userId)
        {
            using (var context = new IUContext())
            {
                ...
                var semesters =
                 ...
                 select new UserSemesterViewModel() { SemesterID = semesterTBLs.SemesterID, SemesterName = semesterTBLs.SemesterName, StartDate = semesterTBLs.StartDate.GetValueOrDefault(), EndDate = semesterTBLs.EndDate.GetValueOrDefault(), SemesterCode = semesterTBLs.SemesterCode };
