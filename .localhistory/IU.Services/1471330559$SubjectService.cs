        ...
        private List<UserSubjectViewModel> GetAllSubjects(string lectureID)
        {
            using (var context = new IUContext())
            {
                ...
                if(lecturerSubjects != null){
                    lecturerSubjects.SubjectTBLs.Select(s => new UserSubjectViewModel() { SubjectID = s.SubjectID, SubjectName = s.SubjectName, AbbreSubjectName = s.AbbreSubjectName });
                }

                return lecturerSubjects.ToList();
