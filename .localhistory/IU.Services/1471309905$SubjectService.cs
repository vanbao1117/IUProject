            ...
            try
            {
                ...
                using (var context = new IUContext())
                {
                    ...
                    foreach (DateTime dateStudy in studyDates)
                    {
                        ...
                        {
                            var classSchedule = new ClassScheduleTBL() { ClassScheduleID = Helper.GenerateRandomId(), ClassID = model.ClassID, RoomID = model.RoomID, SlotID = model.SlotID1 + (model.SlotID2 != null || model.SlotID2.Equals("") ? "" : "-" + model.SlotID2), SubjectID = model.SubjectID.Replace("/r/n", string.Empty), LecturerID = model.LecturerID, StudentListID = studentList.StudentListID, IsAttendance = false, ModeID = model.ModeID, Blog = model.BlogID, DateStudy = dateStudy };
