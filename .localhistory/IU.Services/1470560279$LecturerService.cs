    ...
    public class LecturerService : IDisposable
    {
        ...
        private LectureClassSubjectViewModel GetLectureClassSubject(string userName)
        {
            ...
            using (var context = new IUContext())
            {
                if (userInfo.AccountType.Equals("Admin"))
                {
                    ...
                    var lectures = context.LecturerTBLs.Select(l => new LecturerViewModel() { LecturerID = l.LecturerID, LecturerBirth = l.LecturerBirth, LecturerEmail = l.LecturerEmail, LecturerGender = l.LecturerGender, LecturerName = l.LecturerName, LecturerPhone = l.LecturerPhone, UserID = l.UserID });
        ...
        private PreviewListViewModel GetLecturerPreview(string userName, string classID, string subjectID)
        {
            ...
            using (var context = new IUContext())
            {
                ...
                if (userInfo.AccountType.Equals("Admin"))
                {
                    attandances = AttendanceTBLRepository.FindAllBy(a => a.ClassID == classID && a.SubjectID == subjectID).ToList();
                }
                else
                {
