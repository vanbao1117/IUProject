...
namespace IU.Services
{
    ...
    {
        public async Task<List<LecturerViewModel>> GetAttendanceTwoDaysBeforeSync(string userName)
        {
        ...
        private List<LecturerViewModel> GetAttendanceTwoDaysBefore(string userName)
        {
            using (var context = new IUContext())
            {
                var user = context.AspNetUsers.Where(u => u.UserName == userName).FirstOrDefault();
                var semesterID = GetCurrentSemester().SemesterID;
                var results = context.AttendanceTBLs.Where(x => x.Attendancer == user.Id && x.SemesterID == semesterID).GroupBy(x => new { x.SubjectID, x.ClassID, x.RoomID, x.SlotID }).Distinct().ToList();
