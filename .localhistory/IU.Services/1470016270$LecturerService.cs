    ...
    public class LecturerService : IDisposable
    {
        ...
        private List<UserAttendanceViewModel> GetAttendanceTwoDaysBefore(string userName)
        {
            using (var context = new IUContext())
            {
                ...
                var attendances = results.ToList().Select(a => new UserAttendanceViewModel() { SubjectID = GetSubjectName(a.SubjectID), ClassID = GetClass(a.ClassID), RoomID = a.RoomID, SlotID = a.SlotID, SlotTime = GetSlotbyID(a.SlotID), Note = a.Note});
