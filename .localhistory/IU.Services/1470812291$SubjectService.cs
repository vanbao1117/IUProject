            ...
            try
            {
                ...
                using (var context = new IUContext())
                {
                    var schedule = (from t in (
                       ...
                       select g.OrderBy(p => p.h.DateStudy).FirstOrDefault());

                    var sh = schedule.Select(s=> new ClassScheduleViewModel(){
                            ClassID = classID,
                            LecturerID = s.h.LecturerID,
                            SubjectID = s.h.SubjectID,
                            RoomID = s.h.RoomID,
                            SlotID = s.h.SlotID ,
                            ModeID = s.h.ModeID.Value,
                            BlogID = s.h.Blog.Value,
                            SemesterID = semesterID,
                            DateStudy = s.h.DateStudy,
                            isCreate = false
                    }).GroupBy(x => x.LecturerID).Select(x => x.First());
