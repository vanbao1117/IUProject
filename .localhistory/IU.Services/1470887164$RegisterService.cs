        ...
        public async Task<OpenSubjectViewModel> UndoRegisterSync(OpenSubjectViewModel model, string userName)
        {
            using (var context = new IUContext())
            {
                try
                {
                    ...
                    var class = OpenClassTBLRepository.FindOneBy(s=>s.OpenClassID == model.OpenClassID);
                    ...
                    var stl = StudentListRepository.FindAllBy(s => s.StudentID == student.StudentID && s.ClassID == acceptRegister.);
