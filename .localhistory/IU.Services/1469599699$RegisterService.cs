        ...
        public async Task<OpenSubjectViewModel> AcceptRegisterSync(OpenSubjectViewModel model, string userName)
        {
            using (var context = new IUContext())
            {
                try
                {
                    ...
                    await AcceptRegisterRepository.SaveAsync(new AcceptRegister() { AcceptRegisterID = _id, OpenClassID = model.OpenClassID, OpenSubjectID = model.OpenSubjectID, OrderNum =                     var user = context.AspNetUsers.Where(u => u.UserName == userName).FirstOrDefault();
, RegisterDate = DateTime.Now, StudentID = student.StudentID });
                    return model;
                }
                catch (Exception ex) { }
                return null;
