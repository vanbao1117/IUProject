    ...
    public class SubjectService : IDisposable
    {
        ...
        private bool CreateBis(BisViewModel model, string userName)
        {
            try
            {
                ...
                OpenClassRepository.Save(new OpenClassTBL() { OpenClassID = openClassID, ClassID = classID, CreatedDate = DateTime.Now, Creater = userName, Deadline = model.Deadline, Limit = model.Limit, RoomID = model.RoomID, SemesterID = model.SemesterID, SlotID = string.Join("-", model.SlotIDs) });
        ...
        private List<RoomViewModel> GetRoom(string id)
        {
            using (var context = new IUContext())
            {
