...
namespace IU.Services
{
    ...
    public class Helper
    {
        public IEnumerable<DateTime> GetWeekends(DateTime startDate, DateTime endDate)
        {
        ...
        public static DateTime[] GetStudyDays(DateTime start, DateTime end, string modes, int blog)
        {
            ...
            DateTime[] totalDates = GetWeekends(start, end).ToArray();
            if (blog == 1 && totalDates.Length >= 6)
            {
                _blogStart = FirstDayOfWeek(totalDates[0]);
                _blogEnd = totalDates[5];
            }
            else if (blog == 2 && totalDates.Length > 6)
            {
                _blogStart = FirstDayOfWeek(totalDates[7]);
