...
namespace IU.Services
{
    ...
    {
        public async Task<List<UserAttendanceViewModel>> GetAttendancesSync(string userName)
        {
            using (var context = new IUContext())
            {
                return await Task.Run(() => GetAttendances(userName));
            }
        }

        private List<UserAttendanceViewModel> GetAttendances(string userName)
        {
            List<UserAttendanceViewModel> lsAttendance = new List<UserAttendanceViewModel>();
            using (var context = new IUContext())
            {
                DateTime[] dates = Helper.GetTwoDaysBefore();
