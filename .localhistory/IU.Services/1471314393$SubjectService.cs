...
namespace IU.Services
{
    public class SubjectService : IDisposable
    {
        ...
        private List<SlotViewModel> GetSlot(string slotID)
        {
            using (var context = new IUContext())
            {
                var slots = context.SlotTBLs.Where(s => s.SlotID == slotID).Select(s => new SlotViewModel() { SlotID = s.SlotID, SlotName = s.SlotName, SlotTime = s.SlotTime, NumOfSlot = s.NumOfSlot.Value, TotalSlot = s.TotalSlot.Value });
                return slots.ToList();
            }
        }
