...
namespace IU.Services
{
    public class SubjectService : IDisposable
    {
        ...
        private bool ValidateSchedule(string semesterID, DateTime DateStudy)
        {
            using (var context = new IUContext())
            {
                var sm = GetCurrentSemester(semesterID);
                DateTime[] studyDates = Helper.GetStudyDays(DateTime.Parse(DateStudy), sm.EndDate.Value, mode.Mode, model.BlogID);
            }
        }
