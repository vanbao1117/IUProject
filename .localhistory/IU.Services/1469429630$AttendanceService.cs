        ...
        private UserAttendancePagingViewModel[] GetAttendanceByStudent(int pageNumber, int pageSize, string userName, string semesterCode, string subjectCode)
        {
            using (var context = new IUContext())
            {
                ...
                foreach (SubjectTBL subject in subjects)
                {
                    ...
                    UserAttendancePagingViewModel paging = new UserAttendancePagingViewModel() { TotalPages = totalPage, Page = firstPage.ToArray(), SubjectName = subject.SubjectName };
                    lsPage.Add(paging);

                    if (subjectCode != "" && subjectCode == subject.AbbreSubjectName)
                    {
                        break;
                    }
