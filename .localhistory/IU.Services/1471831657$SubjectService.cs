...
namespace IU.Services
{
    public class SubjectService : IDisposable
    {
        ...
        private bool ValidateSchedule(string semesterID, string dateStudy, string mode, int blog, string room, string slot)
        {
            using (var context = new IUContext())
            {
                var sm = GetCurrentSemester(semesterID);
                DateTime[] studyDates = Helper.GetStudyDays(DateTime.Parse(dateStudy), sm.EndDate.Value, mode, blog);
                foreach (DateTime date in studyDates)
                {
                    context.ClassScheduleTBLs.Where()
                }
            }
        }

        public async Task<bool> ValidateScheduleSync(string semesterID)
        {
            using (var context = new IUContext())
            {
