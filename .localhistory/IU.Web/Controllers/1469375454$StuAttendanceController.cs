...
namespace IU.Web.Controllers
{
    ...
    public class StuAttendanceController : ApiController
    {
        ...
        public async Task<IHttpActionResult> GetAttendanceByStudent(int pageNumber, int pageSize = 20, string semesterName = string.Empty())
        {
            try
            {
                using (AttendanceService _AttendanceService = new AttendanceService())
                {
                    ...
                    var _Attendance = await _AttendanceService.GetAttendanceByStudentSync(pageNumber, pageSize, userName, semesterName);
