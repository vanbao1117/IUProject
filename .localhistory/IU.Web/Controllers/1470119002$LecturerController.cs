...
namespace IU.Web.Controllers
{
    ...
    public class LecturerController : ApiController
    {
        ...
        public async Task<IHttpActionResult> GetTakeAttendances(UserAttendanceViewModel model)
        {
            try
            {
                using (LecturerService _LecturerService = new LecturerService())
                {
                    ...
                    var attendances = await _LecturerService.GetTakeAttendancesSync(model.SubjectID, model.SemesterID, model.ClassID, model.DateStudy, model.isAttendanced);
                    return Ok(attendances);
        ...
        // Post api/Lecturer/GetTakeAttendances
        /// <summary>
        /// GetTakeAttendances
        /// </summary>
        /// <returns></returns>
        ...
        [Authorize]
        [System.Web.Http.HttpPost]
        public async Task<IHttpActionResult> TakeAttendances(List<UserAttendanceViewModel> model)
        {
            try
            {
                using (LecturerService _LecturerService = new LecturerService())
