...
namespace IU.Web.Controllers
{
    ...
    public class StuAttendanceController : ApiController
    {
        ...
        public async Task<IHttpActionResult> GetAttendanceByStudent(int pageNumber, int pageSize = 20)
        {
            try
            {
                using (AttendanceService _AttendanceService = new AttendanceService())
                {
                    ...
                    var _ScheduleClass = await _AttendanceService.GetAttendanceByStudentSync(pageNumber, pageSize, userName);
