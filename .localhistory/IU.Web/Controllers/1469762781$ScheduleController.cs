...
namespace IU.Web.Controllers
{
    ...
    public class ScheduleController : ApiController
    {
        ...
        public async Task<IHttpActionResult> getClass(int pageNumber, int pageSize = 20, string abbreSubjectName = "ALL")
        {
            try
            {
                using (ClassService _ClassService = new ClassService())
                {
                    ...
                    var classs = await _ClassService.GetList(userName);
                    return Ok(classs);
                }
