...
namespace IU.Web.Controllers
{
    ...
    public class ScheduleController : ApiController
    {
        ...
        public async Task<IHttpActionResult> GetAllClassScheduleSync(int pageNumber, int pageSize = 20, string className = "ALL")
        {
            try
            {
                using (ScheduleClassService _ScheduleClassService = new ScheduleClassService())
                {
                    ...
                    else
                    {
                        ...
                        var _ScheduleClass = await _ScheduleClassService.GetClassScheduleSync(pageNumber, pageSize, userName, className);
                        return Ok(_ScheduleClass);
                    }
