        ...
        if ($scope.currentTab == 'activity') {
            ...
            angular.forEach($scope.classSchedules, function (value, key) {
                if (value.edited) {
                    ...
                    var submit = "{ 'oldModel': " + JSON.stringify(oldval) + ", 'newModel': " + JSON.stringify(nval) + " }";


                
                    console.log('Update submit: ', JSON.parse(submit));

                    SubjectService.updateClassSchedule(JSON.parse(submit)).then(
                      function (status) {
                          $timeout(function () {
    ...
    $scope.GetClassSchedule = function () {
        ...
        SubjectService.GetClassSchedule(classID, semesterID).then(
           function (classSchedules) {
               ...
               $scope.oldClassSchedules = classSchedules;
           },
           function (error) {
